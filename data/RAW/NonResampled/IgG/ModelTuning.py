...
### package for imbalance xgboost https://pypi.org/project/imbalance-xgboost/
...
param_space = {'learning_rate': np.logspace(-5, -2,5),
                'n_estimators': [10, 25, 50],
                'subsample': np.linspace(0.1,0.7,3),
                'colsample_bytree':np.linspace(0.05, 0.5, 3),
                'max_depth':[int(x) for x in np.linspace(3,20,3)]}

xgb =  XGBClassifier(objective='binary:logistic', eval_metric='logloss', scale_pos_weight=len(Y==0)/len(Y==1), random_state=RAND) #, scale_pos_weight=len(Y1==0)/len(Y1==1) BinaryFocalLoss
